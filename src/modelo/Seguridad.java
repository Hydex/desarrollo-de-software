...
public class Seguridad {
   ...
   public  boolean verificarTerminal () 
   {
       ...
       try{
             ...
             ResultSet rs = st.executeQuery("SELECT * FROM terminalesMMS  WHERE terminalID='"+terminalID+"'");
             rs.last();
   ...
   public boolean terminalEstaBloqueado() {
        ...
        try
           { 
                ...
                ResultSet rs= st.executeQuery("SELECT Bloqueo FROM terminalesMMS WHERE terminalID='"+terminalID+"'");
                rs.next();
                cTerminal=rs.getInt("Bloqueo");
                rs.close();
            }
            catch (Exception e)
            {
          ...
          try
           {
               ...
               ResultSet rs = st.executeQuery("SELECT * FROM UsuariosHMS WHERE ID='"+usuario+"' AND contraseña "+
                                                                 "='"+password+"'");
               rs.last();
          ...
          try
           {
                ...
                ResultSet rs = st.executeQuery("SELECT * FROM UsuariosHMS WHERE ID='"+usuario+"'AND Estado");
                rs.last();
                acceso1=rs.getRow();
                rs.close();
           }
   ...
   public boolean cuentaEstaBloqueada() {
            try
            {
                   ...
                   ResultSet rsSeg= st.executeQuery("SELECT BloqueoAutomatico FROM UsuariosHMS WHERE ID='"+usuario+"'");
                   boolean verificadorExistencia=rsSeg.next();
                    if(verificadorExistencia){
                     cCuenta=rsSeg.getInt("BloqueoAutomatico");
                    } 
                     rsSeg.close();
             }
            catch (Exception e)
          ...
          try
            {
               ...
               if (this.accesoCuenta()){
                   st.executeUpdate("UPDATE UsuariosHMS SET BloqueoAutomatico=0 WHERE ID='"+usuario+"'");
                   st.executeUpdate("UPDATE terminalesHMS SET Bloqueo=0 WHERE terminalID='"+terminalID+"'");
               }
               else{
                    st.executeUpdate("UPDATE UsuariosHMS SET BloqueoAutomatico=BloqueoAutomatico+1 WHERE ID='"+usuario+"'");
                    st.executeUpdate("UPDATE terminalesHMS SET Bloqueo=bloqueo+1 WHERE terminalID='"+terminalID+"'");
               }
   ...
   public void actualizaPassword(){ 
           try
           {
               ...
               int cambioExitoso = st.executeUpdate("UPDATE UsuariosHMS SET Estado=1 , Contraseña='"+vf_password+"'WHERE ID='"+usuario+"'");
          ...
          try
           {
               ...
               ResultSet rs = st.executeQuery("SELECT NivelAcceso FROM UsuariosHMS WHERE ID='"+usuario+"'");
               rs.next();
               privilegiosS=rs.getString("NivelAcceso");  
