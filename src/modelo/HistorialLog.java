...
//import ConexionAux.ConexionAux;
import java.sql.*;
...
public class HistorialLog{
  ...
  void saveOnHistorialHMS() 
  {
        ...
        try {
            ConexionAux cnHistorial =new ConexionAux();   
            Statement st = cnHistorial.conectar();
               // se guarda la hora, cuando se va a guardar a la base de datos
               //int rsSeg= st.executeUpdate(" insert into historialLoguinHMS values('"+this.getHoraIni()+"','"+this.getHoraFin()+"','"+this.getUser()+"','"+this.getTerminalID()+"','"+this.getTime()+"');");
               ResultSet rs=st.executeQuery("SELECT * FROM Terminal WHERE desTer ='"+this.getTerminalID()+"'");
               rs.next();
               int ideTer = rs.getInt("ideTer");
               int rsSeg= st.executeUpdate("INSERT INTO HistorialLog ( ideUsu , ideTer, horIniHisLog, horFinHisLog) VALUES ('"+this.getUser()+"',"+ideTer+",'"+this.getHoraIni()+"','"+this.getHoraFin()+"')");
